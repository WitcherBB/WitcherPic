...
namespace witcher_pic {
	export class Image;
	export template <bool Copy = false>
...
namespace witcher_pic {
	...
	public:
		...
		auto filter(const ModelMat& model, int rcx, int rcy, FilterType type) -> ImageProcessor& {
			img_->r_matrix_ = imgFilter(img_->r_matrix_, model, rcx, rcy, type);
			if (img_->bpp_ != 8) {
				img_->g_matrix_ = imgFilter(img_->g_matrix_, model, rcx, rcy, type);
				img_->b_matrix_ = imgFilter(img_->b_matrix_, model, rcx, rcy, type);
			}
			return *this;
		}
		...
		auto toRGBA() -> ImageProcessor& {
			if (img_->bpp_ != 32) {
				img_->a_matrix_.fill(255);
				if (img_->bpp_ == 8) {
					img_->g_matrix_ = img_->r_matrix_;
					img_->b_matrix_ = img_->r_matrix_;
				}
			}
			img_->bpp_ = 32;
			return *this;
		...
		auto toGray() -> ImageProcessor& {
			...
			for (size_t i = 0; i < img_->size(); i++) {
				uint8_t gray = static_cast<uint8_t>(
					static_cast<float>(img_->r_matrix_(i)) * 0.299F +
					static_cast<float>(img_->g_matrix_(i)) * 0.587F +
					static_cast<float>(img_->b_matrix_(i)) * 0.114F
				);
				img_->r_matrix_(i) = gray;
			}
			clock_t m_time2 = std::clock();
			std::println("toGray时间: {} ms", m_time2 - m_time1);
			return *this;
		}
		...
		auto toBinary(uint8_t m) -> ImageProcessor& {
			...
			for (auto y = 0u; y < img_->height(); ++y) {
				for (auto x = 0u; x < img_->width(); ++x) {
					img_->r_matrix_(y, x) = img_->r_matrix_(y, x) >= m ? 255 : 0;;
				}
			}
			return *this;
		}
		...
		auto toOtsuBinary() -> ImageProcessor& {
			...
			size_t* gray_table = grayCountTable(img_->r_matrix_);
			double variance[256] = {0};
			auto img_size = img_->size();
			uint8_t L = 0;
		...
		auto grayEnhance(float min_rate, float max_rate) -> ImageProcessor& {
			...
			size_t min_thre = static_cast<size_t>(img_->size() / 100.0 * min_rate);
			size_t max_thre = static_cast<size_t>(img_->size() / 100.0 * max_rate);
			uint8_t min_gray = 0;
			uint8_t max_gray = 0;
			size_t* count_table = grayCountTable(img_->r_matrix_);
			...
			mapGrayImage(img_->r_matrix_, map_table, img_->size());
		...
		auto edgeExtra(SharpenModel model, int index) -> ImageProcessor& {
			...
			auto sharpen_lam = [this](Image* to_sharpen, const ModelMat& model_m) -> Image* {
				to_sharpen->r_matrix_ = imageSharpen(img_->r_matrix_, model_m);
				if (img_->bpp_ != 8) {
					to_sharpen->g_matrix_ = imageSharpen(img_->g_matrix_, model_m);
					to_sharpen->b_matrix_ = imageSharpen(img_->b_matrix_, model_m);
				}
				return to_sharpen;
			};
		...
		auto canny(EdgeInfo& edgeinfo, uint8_t l_threshold, uint8_t h_threshold, unsigned kernelsize = 3,
		           bool l2_gradient = false) -> ImageProcessor& {
		...
		auto tiltCorrection(size_t zoomsize) -> ImageProcessor& {
			ImageProcessor<true> cpy_pro(img_);
			if (cpy_pro.img_->bpp_ != 8) {
				cpy_pro.toGray();
			}
			cpy_pro.canny(50, 100);
			zoomsize = zoomsize ? zoomsize : std::ranges::min(img_->width(), img_->height());
			auto hough = lineExtra(cpy_pro.img_->r_matrix_, zoomsize);
#ifdef _DEBUG
			for (auto i = 0uz; i < hough->size; i++) {
				double theta = hough->max_thetas[i];
				double radius = hough->max_redius[i];
				std::println("θ{0}={1}°, r{0}={2}", i, theta / std::numbers::pi * 180, radius);
		...
		auto addWeighted(const Image& other, float w1, float w2,
			...
			if (img_->bpp_ != other.bpp_) {
				throw std::exception("AddWeighted: Non-uniform bpp!");
			}
			if (img_->width() != other.width() || img_->height() != other.height()) {
				throw std::exception("AddWeighted: Non-uniform size!");
			}

			imageAddWeighted(img_->r_matrix_, w1, other.r_matrix_, w2, r);
			if (img_->bpp_ != 8) {
				imageAddWeighted(img_->g_matrix_, w1, other.g_matrix_, w2, r);
				imageAddWeighted(img_->b_matrix_, w1, other.b_matrix_, w2, r);
			}
