...
namespace witcher_pic {
	...
	public:
		...
		auto filter(const ModelMat& model, int rcx, int rcy,
		...
		auto canny(EdgeInfo& edgeinfo, uint8_t l_threshold, uint8_t h_threshold,
		...
		auto tiltCorrection(size_t zoomsize) -> ImageProcessor& {
			...
			for (size_t i = 0; i < hough->size; i++) {
				...
				printf("θ%lld=%lf°, r%lld=%lf", i, theta / std::numbers::pi * 180, i, radius);
